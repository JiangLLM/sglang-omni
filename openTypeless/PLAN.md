...
Architecture: SwiftUI/AppKit application → private JSON-lines subprocess → owned SGLang-Omni native MLX server on a random loopback port and MLX-LM text processing. The app process owns and cleans up the model server. Existing serving remains unchanged.

Constraints: macOS 14+, Apple Silicon for local inference, Swift Package Manager, Python 3.12 isolated environment. User authorized implementation without confirmation. All new files live here; native ASR code is reused from upstream. Models download from Hugging Face on first preparation. No telemetry. No silent network inference fallback.
